...
## 2. ボードマネージャーURLの追加 → ノード用 esp32パッケージ 3.3.11 のインストール

この章の3.3.11指定はノード用です。`firmware/gateway/gateway.ino` はソース冒頭にesp32 core 2.0系と記載され、ESP-NOWの受信コールバックも2.x用です。gatewayをノードと同じ3.3.11でビルドする手順にはなっていません。集約担当は、実機に書き込んだ環境のesp32パッケージ・M5Unifiedの正確なバージョンを確認してください。gatewayの2.0系の小版番号は、この手順書では未確認です。
...
先に、集約担当の動作済み環境に合わせたesp32 core 2.0系とM5Unifiedを選択します（正確なバージョンは担当者の実環境で確認）。同じIDEでノードの検証に戻るときは、ノード用3.3.11とボード設定に戻してください。
