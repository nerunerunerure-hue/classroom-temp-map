...
## 公開コードの確認と発表準備（2026-09-06時点）

制作期間は9/4で終了し、9/11の発表準備を進めています。現在のmainには[ノード4台のコード](firmware/nodes/)、[M5Stackの表示・SD記録コード](firmware/gateway/)、[CSV取り込みと折れ線グラフ](analysis/)があります。[サンプルの実行手順](analysis/README.md)から分析処理を試せます。サンプルはダミーデータであり、発表の実測結果を再現するものではありません。

[ヒートマップ](analysis/heatmap_temp.py)とその[使用手順](analysis/README.md)もmainに取り込まれています。発表に採用するデータ期間・件数・統計量・最終グラフは分析担当の確定後にそろえます。[発表までの段取り](docs/final-week.md)を参照してください。
