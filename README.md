...
| #47 — CI/CD（GitHub Actions） | コンパイル自動チェック用ワークフロー案の作成・問題指摘 | CI案の検証（検討のうえclosedとして扱い） |
| #50 — 設定（CODEOWNERS） | CODEOWNERS案作成・存在しないアカウント等の検出支援 | 内容確認・運用上の検討（closed） |
...
| #57 — プログラム（集約画面UI） | ラベル表示・描画修正コードの全体生成（Claude Code） | 動作確認・レビュー中（open） |
